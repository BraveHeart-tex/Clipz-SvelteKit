<script lang="ts">
  ...
  let videoFormHandler = new VideoFormHandler({
    modalStore,
    toastStore,
    superFrm,
    storage,
  ...
  {#if $videoFormHandler.videoSrc || data.currentVideo?.url}
    ...
    <div class="grid grid-cols-1 gap-4 mt-4 lg:max-w-[75%]">
      ...
      {#if isEditMode}
        ...
        <VideoUploadArea onChangeHandler={videoFormHandler.handleVideoChange} />
      {/if}
    </div>
    ...
    {/if}
  {:else}
    <UploadVideoStepOne onChangeHandler={videoFormHandler.handleVideoChange} />
  {/if}
</div>
